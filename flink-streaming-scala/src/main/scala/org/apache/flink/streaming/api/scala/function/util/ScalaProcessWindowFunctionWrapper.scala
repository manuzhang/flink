...
final class ScalaProcessWindowFunctionWrapper[IN, OUT, KEY, W <: Window](
  ...
  override def process(key: KEY, input: Iterable[IN],
    ctx: JProcessWindowFunction[IN, OUT, KEY, W]#Context, out: Collector[OUT]): Unit = {
    func.process(key, input.asScala, new func.Context {
      override def window: W = ctx.window()
      override def watermark: Long = ctx.watermark()
    }, out)
  }
